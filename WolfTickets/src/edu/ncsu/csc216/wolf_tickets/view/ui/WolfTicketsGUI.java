...
public class WolfTicketsGUI extends JFrame implements ActionListener {
	...
	private class CategoryPanel extends JPanel implements ActionListener {
		...
		public CategoryPanel() {
			...
			tableTickets.getColumnModel().getColumn(0).setPreferredWidth(7);
			tableTickets.getColumnModel().getColumn(1).setPreferredWidth(3);
			tableTickets.getColumnModel().getColumn(2).setPreferredWidth(300);
			...
			//Row 6 - move up/down
			JPanel btnMoveUpDown = new JPanel();
			btnMoveUpDown.setLayout(new GridLayout(1, 2));
			btnMoveUpDown.add(btnMoveUp);
			btnMoveUpDown.add(btnMoveDown);
			...
			JPanel btnMoveFrontBack = new JPanel();
			btnMoveFrontBack.setLayout(new GridLayout(1, 2));
			btnMoveFrontBack.add(btnMoveToFront);
			btnMoveFrontBack.add(btnMoveToBack);
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			int ticketIdx = tableTickets.getSelectedRow();
			if (e.getSource() == comboCategorys) {
				...
				try {
					...
					group.addCategory(new edu.ncsu.csc216.wolf_tickets.model.category.TicketCategory(categoryName, 0));
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, iae.getMessage());
				}
			} else if (e.getSource() == btnEditCategory) {
				try {
					if (group.getCurrentCategory() instanceof ActiveTicketCategory) {
				...
				try {
					group.getCurrentCategory().getTickets().moveUp(ticketIdx);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, iae.getMessage());
				...
				try {
					group.getCurrentCategory().getTickets().moveDown(ticketIdx);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, iae.getMessage());
				} catch (IndexOutOfBoundsException ioobe) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, "No ticket selected.");
				...
				try {
					group.getCurrentCategory().getTickets().moveToFront(ticketIdx);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, iae.getMessage());
				} catch (IndexOutOfBoundsException ioobe) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, "No ticket selected.");
				...
				try {
					group.getCurrentCategory().getTickets().moveToBack(ticketIdx);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, iae.getMessage());
				} catch (IndexOutOfBoundsException ioobe) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, "No ticket selected.");
		...
		public void updateCategorys() {
			if (group == null) {
				...
				String [] categoryNames = group.getCategoryNames();
			...
			tableTickets.getColumnModel().getColumn(0).setPreferredWidth(10);
			tableTickets.getColumnModel().getColumn(1).setPreferredWidth(3);
			tableTickets.getColumnModel().getColumn(2).setPreferredWidth(250);
		}
		...
		public void enableButtons(boolean enable) {
			...
			if (pnlTicket != null) {
				if (enable) {
					pnlTicket.setAddSaveButtonText(enable);
				} else {
					pnlTicket.enableButtons(enable);
				}
			}
		}
		...
		private class TicketTableModel extends AbstractTableModel {
			...
			private String [] columnNames = {"Category", "Owner", "Ticket Title"};
	...
	private class TicketPanel extends JPanel implements ActionListener {
		...
		public TicketPanel() {
			...
			//Row 7 - complete/clear
			JPanel row7 = new JPanel();
			row7.setLayout(new GridLayout(1, 2));
			row7.add(btnComplete);
			row7.add(btnClear);
		...
		/**
		 * Performs action based on ActionEvent.
		 * @param e ActionEvent that causes the event that the JPanel is listening for.
		 */
		@Override
		public void actionPerformed(ActionEvent e) {
			int idx = pnlCategory.tableTickets.getSelectedRow();
			
			if (e.getSource() == btnAddSave) {
				try {
					if (idx == -1) {
						Ticket t = new Ticket(txtTicketName.getText(), txtDescription.getText(), txtOwner.getText(), checkActive.isSelected());
						group.addTicket(t);
					} else {
						group.editTicket(idx, txtTicketName.getText(), txtDescription.getText(), txtOwner.getText(), checkActive.isSelected());
					}
					setTicket(-1);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(WolfTicketsGUI.this, iae.getMessage());
		...
		public void setTicket(int idx) {
			try {
				...
				if (group.getCurrentCategory() instanceof ActiveTicketCategory) {
					...
					enableButtons(true); //edit selected ticket
					setAddSaveButtonText(false);
				}
				...
				enableButtons(false); //set up for a possible add
				setAddSaveButtonText(!(group.getCurrentCategory() instanceof ActiveTicketCategory));
			}
		}
		...
		/**
		 * Enable or disable add/edit button
		 * @param isAdd true if add text, false if edit text
		 */
		public void setAddSaveButtonText(boolean isAdd) {
			if (isAdd) {
